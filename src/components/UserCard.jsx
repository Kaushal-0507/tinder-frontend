  ...
  return (
    <div className="w-88 max-h-[500px] bg-gray-900 rounded-2xl overflow-hidden shadow-xl relative border border-gray-700 mx-auto my-4">
      ...
      <div className="absolute bottom-20 left-5 text-white ">
        ...
        <p className="text-gray-300 text-sm line-clamp-2 mb-0.5">{about}</p>
        <div className="flex flex-wrap gap-2">
          {hobbies?.map((hobby) => (
            <span
              key={hobby}
              className="bg-white/10 text-gray-300 px-2 py-1 rounded-md text-[11px] border border-gray-600"
            >
              {hobby}
            </span>
          ))}
      ...
      <div className="absolute bottom-4 left-0 right-0 flex justify-between items-center px-6">
        {/* Refresh Button */}
        <button className="w-12 cursor-pointer h-12 rounded-full bg-gray-800 shadow-md flex items-center justify-center text-yellow-400 hover:bg-gray-700 transition-colors border border-gray-700">
        ...
        {/* More Options Button */}
        <button className="w-12 h-12 cursor-pointer rounded-full bg-gray-800 shadow-md flex items-center justify-center text-gray-400 hover:bg-gray-700 transition-colors border border-gray-700">
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
